...
public class NetworkManagerT extends AbstractMauTransformer {
	...
	public byte[] transform(byte[] bytes, boolean obf) {
		...
		String packet = obf ? "id" : "net/minecraft/network/Packet", playerPacket = obf ? "mg" : "net/minecraft/network/play/client/C03PacketPlayer";
		...
		for (MethodNode m : node.methods) {
			if (m.name.equals(methodName) && m.desc.equals(desc)) {
				...
				inject.add(new FieldInsnNode(Opcodes.GETSTATIC, ActionFreecam.class.getName().replace('.', '/'), "active", "Z"));
				LabelNode ln1 = new LabelNode();
				inject.add(new JumpInsnNode(Opcodes.IFEQ, ln1));
				inject.add(new VarInsnNode(Opcodes.ALOAD, 1));
				inject.add(new TypeInsnNode(Opcodes.INSTANCEOF, playerPacket));
				inject.add(new JumpInsnNode(Opcodes.IFEQ, ln1));
				...
				inject.add(new JumpInsnNode(Opcodes.IFEQ, ln2));
				inject.add(new VarInsnNode(Opcodes.ALOAD, 1));

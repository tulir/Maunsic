...
/**
 * An exception indicating a problem in parsing an expression. It can produce a short, cryptic error message (with getMessage()) or a long, hopefully helpful
 * one (with explain()).
 */
public class SyntaxException extends Exception {
	private static final long serialVersionUID = 6593167190855919067L;

	/**
	 * An error code meaning the input string couldn't reach the end of the input; the beginning constituted a legal expression, but there was unparsable stuff
	 * left over.
	 */
	public static final int INCOMPLETE = 0;
	
	/**
	 * An error code meaning the parser ran into a non-value token (like "/") at a point it was expecting a value (like "42" or "x^2").
	 */
	public static final int BAD_FACTOR = 1;
	
	/**
	 * An error code meaning the parser hit the end of its input before it had parsed a full expression.
	 */
	public static final int PREMATURE_EOF = 2;
	
	/**
	 * An error code meaning the parser hit an unexpected token at a point where it expected to see some particular other token.
	 */
	public static final int EXPECTED = 3;
	
	/**
	 * An error code meaning the expression includes a variable not on the `allowed' list.
	 */
	 ...
	 * @param expected if nonnull, the token the parser expected to see (in place of the erroneous token it did see)
	 */
	...
	private void explainWhere(StringBuffer sb) {
		if (scanner.isEmpty()) sb.append("It's empty!\n");
		else if (scanner.atStart()) {
			sb.append("It starts with ");
			quotify(sb, theToken());
			if (isLegalToken()) sb.append(", which can never be the start of a formula.\n");
			else sb.append(", which is a meaningless symbol to me.\n");
		} else {
			sb.append("I got as far as ");
			quotify(sb, asFarAs());
			sb.append(" and then ");
			if (scanner.atEnd()) sb.append("reached the end unexpectedly.\n");
			else {
				sb.append("saw ");
				quotify(sb, theToken());
				if (isLegalToken()) sb.append(".\n");
				else sb.append(", which is a meaningless symbol to me.\n");
			}
		}
	}
	...
	private void explainWhy(StringBuffer sb) {
		switch (reason) {
			case INCOMPLETE:
				if (isLegalToken()) sb.append("The first part makes sense, but I don't see " + "how the rest connects to it.\n");
				break;
			case BAD_FACTOR:
			case PREMATURE_EOF:
